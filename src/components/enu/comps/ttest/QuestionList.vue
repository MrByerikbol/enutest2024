    ...
    <b-row>
        <b-col lg="12">
                    ...
                    <b-row class="mt-2">
                        <b-col lg="6">
                            <select 
                                ...
                                v-model="groupId" class="form-sm-control w-100">
                                <option value=0>{{$t('enu.ttest.questionList.chooseGroupTxt')}}</option>
                                <!-- uudam tengeriin tsenher mandal deer usan mornii zuulun dolgio horvoog uyraah duutai bilee hongor chamdaa duugaan duulaya 
                                mini huuruhun saran , hairiin mini od chi saran nadad chi mini nar saran hairt mini chi saran -->
                        ...
                        <b-col lg="6">
                            <select 
                                ...
                                <option v-for="(c,i) in testCats.filter(c=>parseInt(c.groupId)==parseInt(groupId))"
                                    :key="i" :value="c.catId">
            ...
            <b-row>
                ...
                <AddQuestion :groups="groups" :refreshRecordTable="refreshRecordTable"/>
            </b-row>
...
import AddQuestion from '@/components/enu/modal/ttest/AddQuestion';
import {EventBus} from '@/EventBus.js';
import axios from 'axios';
import Vue from 'vue';
export default {
    name : "QuestionList",
    props:['groups',"prefreshRecordTable"],
    ...
    methods:{
        ...
        updateRecord(){
            ...
            if(this.selectedRows.length==1){
                ...
                if(id && id>0){
                    ...
                    axios.get(apiDomain+'/admin/enu/ttest/updatequestion/'+id+'',{headers:getHeader()})
                    .then(response=>{
                        let rData = response.data;
                        EventBus.$emit('updateQuestion',rData);
        ...
        recordProvider(ctx){
            //alert("yes");
            this.isBusy = true;
            let promise = axios.post(apiDomain+'/admin/enu/ttest/gettestquestionlist',ctx,{headers:getHeader()});
    ...
    created(){
         axios.post(apiDomain+'/admin/enu/ref/getdatalist',{refType:'category'},{headers:getHeader()})
            .then(response=>{
