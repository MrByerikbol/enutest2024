<template> 
    <b-row>
        <b-col lg="12" class="py-3">
            <h3>
                <span class="bd-content-title">{{$t('enu.documentModule.title')}}</span>
            </h3>
        </b-col>
        <b-col lg="5">
            <b-col lg="12">
                <h5>
                    <span class="bd-content-title">{{$t('enu.documentCategory.title')}}</span>
                </h5>
            </b-col>
            <b-col lg="12" class="mb-3">
                ...
                <b-button variant="warning" size="sm" class="float-right" v-b-modal.blockModal>{{$t('enu.blockList.title')}}</b-button>
            </b-col>
            <b-row>
                <b-col lg="8">
                    <b-form-input
        ...
        <b-col lg="7">
         ...
         <AddDocumentCategory :refreshRecordTable="refreshRecordTable"/>    
         <DocumentBlock/>    
    </b-row>
...
import DocumentList from "@/components/enu/comps/DocumentList";
import AddDocumentCategory from "@/components/enu/modal/AddDocumentCategory";
import DocumentBlock from "@/components/enu/comps/DocumentBlock";
    ...
    data(){
        return{
            fields: [
                ...
                {
                    key: 'docCount',
                    label: 'enu.docCatList.docCount',
                },
                {
                    key: 'maxPoint',
                    label: 'enu.docCatList.maxPoint',
                    variant:'danger'
                },
                {
                    key: 'blockName',
    ...
    methods:{
        ...
        updateRecord(){
            if(this.selectedRows.length==1){
                let id = this.selectedRows[0].docCatId;
                if(id && id>0){
                    axios.post(apiDomain+'/admin/enu/ref/getref',{refType:'documentCategory',refId:id},{headers:getHeader()})
                    .then(response=>{
                        let rData = response.data;
                        EventBus.$emit('documentCategory',rData);
        ...
        recordProvider(ctx){
            //alert("yes");
            this.isBusy = true;
            let promise = axios.post(apiDomain+'/admin/enu/kpi/documentcatlist',ctx,{headers:getHeader()});
