<template>    
    <b-col lg="12">
        <b-col lg="12">
            <h5>
                <span class="bd-content-title">{{$t('enu.departmentList.title')}}</span>
            </h5>
        </b-col>
...
export default {
    ...
    data(){
        return{
            fields: [
                ...
                {
                    key: 'departmentName',
                    label: 'enu.departmentList.departmentName',
                },
            ],
            isSmall:false,
            isBusy:false,
    ...
    methods:{
        ...
        recordProvider(ctx){
            //alert("yes");
            this.isBusy = true;
            let promise = axios.post(apiDomain+'/admin/enu/kpi/departmentlist',ctx,{headers:getHeader()});
